...
<script>
    ...
    //快速查询
    function vague_query() {
        var condition = $("#search_news").val();
        if (condition.length == 0 || condition == "" || condition == null){
            news_table.ajax.reload();
        }
        $('#news_table').DataTable().ajax.url("fast_query_news?condition="+condition).load();
    }
</script>
<!-- 增加 -->
                ...
                <div class="modal-body">
                    <div class="box-body">
                        <form id="edit_form" enctype="multipart/form-data">
                            <div style="width:100%; margin:0 auto;">
                                <input name="content" id="edit_content" type="hidden"/>
                                <input type="hidden" name="id" id="edit_id">
                                标题：<input style="width:600px; height:30px; line-height:30px; font-size:18px; " name="title" id="edit_title"><br><br>
                                来源：<input style="width:600px; height:30px; line-height:30px; font-size:18px; " name="source" id="edit_source"><br>
                                <br>
                                图片：<input type="file" name="pic" id="edit_pic" accept="image/*" /> <br><br>
        ...
        <div class="btn-toolbar list-toolbar">
            ...
            <div class="col-lg-6" style="float:right;width:280px;margin-right:50px">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="快速搜索新闻..." id="search_news" onkeyup="vague_query()">
                </div><!-- /input-group -->
            </div><!-- /.col-lg-6 -->
        </div>
...
<script type="text/javascript">
    ...
    document.getElementById('edit').addEventListener('click', function () {
        ...
        var source = $("#edit_source").val().trim();
        var pic = $("#edit_pic").val().trim();
        var news_data = editor_edit.txt.html();
        if (title.length == 0 || title == "") {
            layer.msg("请填写标题");
            $("#edit_title").focus();
            return;
